/* RetiredHomeLoans match form — one big friendly question per screen.
...
const MATCH_RULES = [
  { match: (a) => a.income === "rentals", name: "DSCR Investor Loan" },
  { match: (a) => a.credit === "event", name: "Fresh Start Loan" },
  { match: () => true, name: "Asset-Based Purchase Loan" },
];
function matchedProduct() {
  return MATCH_RULES.find((r) => r.match(answers)).name + " — RetiredHomeLoans";
}
...
const STEPS = [
  {
    ...
    choices: [
      { value: "downsize", label: "Downsizing", sub: "Less house, less upkeep" },
      { value: "relocate", label: "Moving near family", sub: "A new state or town" },
      { value: "second", label: "Buying a second home", sub: "Somewhere warm, somewhere cool" },
      { value: "explore", label: "Just exploring", sub: "Seeing what I'd qualify for" },
    ],
  },
  {
    key: "property",
    question: "How will you use the new home?",
    choices: [
      { value: "primary", label: "It'll be my main home" },
      { value: "second", label: "A second home" },
      { value: "investment", label: "A rental property" },
    ],
  },
  {
    key: "amount",
    question: "Roughly what price range are you shopping in?",
  ...
  {
    key: "income",
    question: "Where does your money come from these days?",
    help: "Pick the biggest one — specialists can combine several.",
    choices: [
      { value: "retired", label: "Social Security & pension", sub: "Steady monthly checks" },
      { value: "assets", label: "Savings & investments", sub: "IRA, 401(k), brokerage — money you draw on" },
      { value: "rentals", label: "Rental income", sub: "Tenants pay you rent" },
      { value: "working", label: "Still working, at least a little", sub: "Part-time, consulting, a business" },
    ],
  },
  {
    key: "credit",
